...
std::string MermaidVisitor::next_id() { 
    return "node_" + std::to_string(node_count++); 
}

std::string MermaidVisitor::format_loc(const SourceLocation& loc) {
    return "\n<font size='2' color='#666'>📍 [" + 
           loc.file_path + ":" + std::to_string(loc.line) + ":" + std::to_string(loc.column) + "]</font>";
}

void MermaidVisitor::add_node(const std::string& id, const std::string& label, 
    ...
    if (!parent_stack.empty()) {
        ...
        out << "  " << parent_stack.top() 
            << " -- \" <font color='black'><b>" << text << "</b></font> \" ----> " 
            << id << "\n";
    }
}

void MermaidVisitor::visit_child(NodeAST& node, const std::string& current_id, 
...
std::string MermaidVisitor::op_to_str(TokenType type) {
    switch (type) {
...
MermaidVisitor::MermaidVisitor(std::ostream& output) : out(output) {}

void MermaidVisitor::start() {
    out << "%%{init: { 'theme': 'base', 'themeVariables': { 'primaryColor': '#ffffff', "
        << "'primaryTextColor': '#000000', 'lineColor': '#333333' }, "
        << "'graph': {'rankSep': 120, 'nodeSep': 100}} }%%\n";
    out << "graph TD\n";
...
void MermaidVisitor::visit(ProgramNode& node) {
...
void MermaidVisitor::visit(BlockNodeAST& node) {
    std::string id = next_id();
    add_node(id, "📂 БЛОК КОДА { }", node.location, "[ ]");
    for (auto& stmt : node.statements) visit_child(*stmt, id, "строка");
}

void MermaidVisitor::visit(ExpressionStatementNodeAST& node) {
    if (node.expression) node.expression->accept(*this);
}

void MermaidVisitor::visit(MakeNodeAST& node) {
    std::string id = next_id();
...
void MermaidVisitor::visit(FunctionNodeAST& node) {
    std::string id = next_id();
    ...
    for (auto& param : node.parameters) {
        ...
        std::string label = "АРГ: " + param.name + (param.is_variadic ? "..." : "") 
                          + " (" + param.type + ")";
...
void MermaidVisitor::visit(BinaryOperationNodeAST& node) {
    std::string id = next_id();
...
void MermaidVisitor::visit(UnaryOperationNodeAST& node) {
    std::string id = next_id();
    add_node(id, op_to_str(node.op), node.location, "([ ])");
    out << "  class " << id << " op_style\n";
...
void MermaidVisitor::visit(TernaryOperationNodeAST& node) {
    std::string id = next_id();
...
void MermaidVisitor::visit(LiteralNodeAST& node) {
    ...
    add_node(next_id(), "💎 " + node.value + " (" + type_str + ")", 
             node.location, "[]");
}

void MermaidVisitor::visit(IdentifierNodeAST& node) {
    add_node(next_id(), "🔍 " + node.name, node.location, "([ ])");
}

void MermaidVisitor::visit(WhileNodeAST& node) {
    std::string id = next_id();
...
void MermaidVisitor::visit(RangeOperationNodeAST& node) {
    std::string id = next_id();
...
void MermaidVisitor::visit(ForNodeAST& node) {
    std::string id = next_id();
...
void MermaidVisitor::visit(BreakNodeAST& node) {
    add_node(next_id(), "🛑 ПРЕРВАТЬ (break)", node.location, "[[ ]]");
}

void MermaidVisitor::visit(ContinueNodeAST& node) {
    add_node(next_id(), "⏭️ ПРОДОЛЖИТЬ (continue)", node.location, "[[ ]]");
}

void MermaidVisitor::visit(TryNodeAST& node) {
...
void MermaidVisitor::visit(ClassNodeAST& node) {
    std::string id = next_id();
...
void MermaidVisitor::visit(MatchNodeAST& node) {
...
void MermaidVisitor::visit(TestNodeAST& node) {
    std::string id = next_id();
...
void MermaidVisitor::visit(AssertNodeAST& node) {
    std::string id = next_id();
